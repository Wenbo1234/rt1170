...
void LCDIFV2_SetLayerBufferConfig(LCDIFV2_Type *base, uint8_t layerIndex, const lcdifv2_buffer_config_t *config)
{
    ...
    DbgConsole_Printf(" pixelFormat=0x%x\n", (uint32_t)config->pixelFormat);

    DbgConsole_Printf(" CTRLDESCL1=0x%x\n", base->LAYER[layerIndex].CTRLDESCL1);
    DbgConsole_Printf(" CTRLDESCL2=0x%x\n", base->LAYER[layerIndex].CTRLDESCL2);
    DbgConsole_Printf(" CTRLDESCL3=0x%x\n", base->LAYER[layerIndex].CTRLDESCL3);
    DbgConsole_Printf(" CTRLDESCL4=0x%x\n", base->LAYER[layerIndex].CTRLDESCL4);
    DbgConsole_Printf(" CTRLDESCL5=0x%x\n", base->LAYER[layerIndex].CTRLDESCL5);
    DbgConsole_Printf(" CTRLDESCL6=0x%x\n", base->LAYER[layerIndex].CTRLDESCL6);

    DbgConsole_Printf(" base->HSYN_PARA=0x%x\n", base->HSYN_PARA);
    DbgConsole_Printf(" base->VSYN_PARA=0x%x\n", base->VSYN_PARA);

    reg = (reg & ~(LCDIFV2_CTRLDESCL5_AB_MODE_MASK) | LCDIFV2_CTRLDESCL5_AB_MODE(3));

    //reg = (reg & ~(LCDIFV2_CTRLDESCL5_GLOBAL_ALPHA_MASK) | LCDIFV2_CTRLDESCL5_GLOBAL_ALPHA(0xcf));
    //reg = (reg & ~(LCDIFV2_CTRLDESCL5_GLOBAL_ALPHA_MASK) | LCDIFV2_CTRLDESCL5_GLOBAL_ALPHA(0x8f));
    reg = (reg & ~(LCDIFV2_CTRLDESCL5_GLOBAL_ALPHA_MASK) | LCDIFV2_CTRLDESCL5_GLOBAL_ALPHA(0x1));
