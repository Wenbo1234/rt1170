...
serial_manager_status_t Serial_UsbCdcInit(serial_handle_t serialHandle, void *serialConfig)
{
    ...
    serialUsbCdc->attach = 0;
    serialUsbCdc->cdcAcmHandle = (class_handle_t)NULL;
    serialUsbCdc->deviceHandle = NULL;
    ...
    vTaskDelay( pdMS_TO_TICKS( 600UL ) );
...
serial_manager_status_t Serial_UsbCdcWrite(serial_handle_t serialHandle, uint8_t *buffer, uint32_t length)
{
    ...
    if (serialUsbCdc->attach == 0 || serialUsbCdc->startTransactions == 0)
    {
        return kStatus_SerialManager_DcdNotReady;
    }
...
bool Serial_UsbCdcIsReady(serial_handle_t serialHandle)
{
    serial_usb_cdc_state_t *serialUsbCdc;
    ...
    return (serialUsbCdc->attach == 1) && (serialUsbCdc->startTransactions == 1);
}
#endif
