...
int wifi_send_hs_cfg_cmd(enum wlan_bss_type type, t_u32 ipv4_addr, t_u16 action, t_u32 conditions);
...
extern void sniffer_register_callback(void (*sniffer_cb)(const wlan_frame_t *frame, const uint16_t len));
extern void sniffer_deregister_callback();

extern void rx_mgmt_register_callback(void (*rx_mgmt_cb)(const enum wlan_bss_type bss_type,
...
static int wlan_send_host_sleep()
{
    ...
    return wifi_send_hs_cfg_cmd(type, ipv4_addr, HS_CONFIGURE,
...
static int security_profile_matches(const struct wlan_network *network, const struct wifi_scan_result *res)
{
    ...
    if (config->type == WLAN_SECURITY_WPA2 || config->type == WLAN_SECURITY_OWE_ONLY)
        return res->WPA_WPA2_WEP.wpa2;
    ...
    if (network->ssid_specific)
    {
        if ((!res->ssid_len) || ((memcmp(network->ssid, (char *)res->ssid, res->ssid_len)) &&
                                 ((res->trans_mode == OWE_TRANS_MODE_OWE) &&
                                  (memcmp(network->trans_ssid, (char *)res->ssid, res->ssid_len)))
...
static void do_scan(struct wlan_network *network)
{
    ...
    else
    {
        if (channel)
        {
            ...
            ret = wifi_send_scan_cmd(type, bssid, ssid, bridge_ssid, 1, chan_list, 0, false);
        }
        else
        {
            ret = wifi_send_scan_cmd(type, bssid, ssid, NULL, 0, NULL, 0, false);
        }
    }
    if (ret)
    {
...
static int do_start(struct wlan_network *network)
{
    ...
    if (network->role == WLAN_BSS_ROLE_UAP)
    {
        ...
        ret = wifi_uap_start(network->type, network->ssid,
                             ...
                             wlan.mac,
#endif
                             network->security.type, &network->security.psk[0], network->channel, wlan.scan_chan_list);
        if (ret)
        {
...
static void do_connect_failed(enum wlan_event_reason reason)
{
    ...
    wlcm_w("connecting to \"%s\" failed", wlan.networks[wlan.cur_network_idx].name);
...
static void start_association(struct wlan_network *network, struct wifi_scan_result *res)
{
    int ret;
...
static void handle_scan_results(void)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (ret == WM_SUCCESS && network_matches_scan_result(network, res))
        {
...
static void wlcm_process_channel_switch(enum cm_sta_state *next, struct wlan_network *network)
{
    if (is_state(CM_STA_CONNECTED))
    {
                                              ...
                                              struct wlan_network *network)
{
    ...
    PRINTF("channel=%d,chanBand=%d,chanWidth=%d,chan2Offset=%d,scanMode=%d", pnewNode->chanBand.chanNum,
...
static void wlcm_request_disconnect(enum cm_sta_state *next, struct wlan_network *curr_nw)
{
    ...
    if (
        ...
        (!wps_session_attempt) &&
#endif
        (wlan.sta_state < CM_STA_IDLE || is_state(CM_STA_IDLE) || is_state(CM_STA_DEEP_SLEEP) || is_state(CM_STA_PDN)))
    {
        os_rwlock_read_unlock(&ps_rwlock);
        wifi_set_xfer_pending(false);
...
int wlan_ieeeps_on(unsigned int wakeup_conditions)
{
    ...
    if (wlan.cm_ieeeps_configured || wlan.cm_deepsleepps_configured || wlan.cm_pdnps_configured)
        return WLAN_ERROR_STATE;
...
int wlan_sniffer_start(const uint16_t filter_flags,
                       const uint8_t radio_type,
                       const uint8_t channel,
                       void (*sniffer_callback)(const wlan_frame_t *frame, const uint16_t len))
{
...
int wlan_get_current_data_rate(wlan_data_rate_t *data_rate)
{
    return wifi_send_get_datarate(data_rate);
}

int wlan_get_tbtt_offset_stats(wlan_tbtt_offset_t *tbtt_offset)
{
    return wifi_send_get_tbtt_offset(tbtt_offset);
}
...
int wlan_rx_mgmt_indication(const enum wlan_bss_type bss_type,
                            ...
                            void (*rx_mgmt_callback)(const enum wlan_bss_type bss_type,
                                                     const wlan_mgmt_frame_t *frame,
                                                     const size_t len))
{
