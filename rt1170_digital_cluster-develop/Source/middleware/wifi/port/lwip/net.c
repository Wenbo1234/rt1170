...
int net_wlan_init(void)
{
  ...
  ret = netifapi_netif_add(&g_wfd.netif, ip_2_ip4(&g_wfd.ipaddr),
					 ip_2_ip4(&g_wfd.ipaddr),
					 ip_2_ip4(&g_wfd.ipaddr), NULL,
		if (ret) {
...
int net_get_if_ipv6_addr(struct wlan_ip_config *addr, void *intrfc_handle)
{
    ...
    {
        memcpy(addr->ipv6[i].address, ip_2_ip6(&(if_handle->netif.ip6_addr[i]))->addr, 16);
...
int net_get_if_ipv6_pref_addr(struct wlan_ip_config *addr, void *intrfc_handle)
{
    ...
    for (i = 0; i < MAX_IPV6_ADDRESSES; i++)
    {
        ...
        {
            memcpy(addr->ipv6[ret++].address, ip_2_ip6(&(if_handle->netif.ip6_addr[i]))->addr, 16);
        }
    }
    return ret;
}
