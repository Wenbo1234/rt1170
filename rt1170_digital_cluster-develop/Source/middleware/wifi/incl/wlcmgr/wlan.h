...
static inline int is_valid_security(int security)
{
    ...
    if ((security == WLAN_SECURITY_NONE) || (security == WLAN_SECURITY_WEP_OPEN) || (security == WLAN_SECURITY_WPA) ||
        ...
        (security == WLAN_SECURITY_EAP_TLS) || (security == WLAN_SECURITY_PEAP_MSCHAPV2) ||
        (security == WLAN_SECURITY_OWE_ONLY) || (security == WLAN_SECURITY_WPA3_SAE) ||
...
typedef wifi_data_rate_t wlan_data_rate_t;
 ...
 *
 * \param[in] data_rate A pointer to structure which will have
 ...
 * from Wi-Fi firmware.
 *
 * \param[in] tbtt_offset A pointer to structure which will hold
 ...
 *
 * \param[in] A pointer to structure where stats collected from Wi-Fi firmware
 *	      will be copied.
 *
 ...
 * \param[in] wakeup_conditions conditions to wake the processor up on. This should
 *            be a logical OR of the conditions in \ref wlan_wakeup_event_t.
 *            Typically devices would want to wake up on
 *            \ref WAKE_ON_ALL_BROADCAST,
 *            \ref WAKE_ON_UNICAST,
 ...
 *
 */
void pm_uapps_hs_cfg(char enabled,
 ...
 *
 */
int wlan_inject_frame(const enum wlan_bss_type bss_type, const uint8_t *buff, const size_t len);
