                                             ...
                                             IN mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf != MNULL)
    {
        ...
        memset(pmpriv->adapter, &pget_info->param.signal, 0, sizeof(mlan_ds_get_signal));
                                               ...
                                               IN mlan_ioctl_req *pioctl_buf)
{
    ...
    memcpy(pmpriv->adapter, pmpriv->curr_addr, pmac_addr->mac_addr, MLAN_MAC_ADDR_LENGTH);
    ...
    if (pioctl_buf)
    {
        ...
        memcpy(pmpriv->adapter, &bss->param.mac_addr, pmpriv->curr_addr, MLAN_MAC_ADDR_LENGTH);
...
#define WAPI_KEY_SIZE 32
            switch (wlan_le16_to_cpu(pkey->key_param_set.key_type_id))
            {
                case KEY_TYPE_ID_WEP:
                    ...
                    memcpy(pmpriv->adapter, sec->param.encrypt_key.key_material, &pkey->key_param_set.key[2],
                           sec->param.encrypt_key.key_len);
                    break;
                case KEY_TYPE_ID_TKIP:
                    sec->param.encrypt_key.key_len = wlan_le16_to_cpu(pkey->key_param_set.key_len);
                    memcpy(pmpriv->adapter, sec->param.encrypt_key.key_material, pkey->key_param_set.key,
                           sec->param.encrypt_key.key_len);
                    break;
                ...
                case KEY_TYPE_ID_WAPI:
                    ...
                    memcpy(pmpriv->adapter, sec->param.encrypt_key.key_material, &pkey->key_param_set.key[2],
                           sec->param.encrypt_key.key_len);
                    memcpy(pmpriv->adapter, sec->param.encrypt_key.pn, &pkey->key_param_set.key[2 + WAPI_KEY_SIZE],
                           PN_SIZE);
                    break;
            }
        }
            ...
            while (tlv_buf_len > 0)
            {
                ...
                switch (tlv)
                {
                    case TLV_TYPE_SSID:
                        ...
                        memcpy(pmpriv->adapter, sec->param.passphrase.ssid.ssid, pssid_tlv->ssid,
                    ...
                    case TLV_TYPE_BSSID:
                        ...
                        memcpy(pmpriv->adapter, &sec->param.passphrase.bssid, pbssid_tlv->bssid, MLAN_MAC_ADDR_LENGTH);
                    ...
                    case TLV_TYPE_PASSPHRASE:
                        ...
                        memcpy(pmpriv->adapter, sec->param.passphrase.psk.passphrase.passphrase,
                               passphrase_tlv->passphrase, MIN(MLAN_MAX_PASSPHRASE_LENGTH, passphrase_tlv->header.len));
                    ...
                    case TLV_TYPE_PMK:
                        ...
                        memcpy(pmpriv->adapter, sec->param.passphrase.psk.pmk.pmk, ppmk_tlv->pmk,
...
static mlan_status wlan_ret_ibss_coalescing_status(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *resp)
{
    ...
    {
        /* BSSID */
        memcpy(pmpriv->adapter, pmpriv->curr_bss_params.bss_descriptor.mac_address, pibss_coal_resp->bssid,
               MLAN_MAC_ADDR_LENGTH);
                                         ...
                                         OUT mlan_ioctl_req *pioctl_buf)
{
    ...
    if (cust_ie)
    {
        ...
        memcpy(pmpriv->adapter, &misc->param.cust_ie, cust_ie, (cust_ie->len + sizeof(MrvlIEtypesHeader_t)));
        if (max_mgmt_ie)
        {
            ...
            if (max_mgmt_ie->type == TLV_TYPE_MAX_MGMT_IE)
            {
                ...
                memcpy(pmpriv->adapter, (t_u8 *)&misc->param.cust_ie + (cust_ie->len + sizeof(MrvlIEtypesHeader_t)),
                       max_mgmt_ie, max_mgmt_ie->len + sizeof(MrvlIEtypesHeader_t));
            }
        }
    }
    ...
    if (pioctl_buf)
    {
        ...
        switch (type)
        {
            ...
            case HostCmd_CMD_802_11_EEPROM_ACCESS:
            {
                ...
                {
                    memcpy(pmadapter, &eeprom->value, &cmd_eeprom->value, MIN(MAX_EEPROM_DATA, eeprom->byte_count));
                                          ...
                                          IN mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf && (pioctl_buf->action == MLAN_ACT_GET))
    {
        ...
        memcpy(pmpriv->adapter, user_data->user_data, cmd_user_data->user_data, user_data->user_data_length);
