...
static void wlan_enable_aes_key(pmlan_private pmpriv)
{
    ...
    memcpy(pmpriv->adapter, encrypt_key.key_material, pmpriv->aes_key.key_param_set.key, encrypt_key.key_len);
...
static mlan_status wlan_get_info_bss_info(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    /* SSID */
    memcpy(pmadapter, &info->param.bss_info.ssid, &pbss_desc->ssid, sizeof(mlan_802_11_ssid));

    /* BSSID */
    memcpy(pmadapter, &info->param.bss_info.bssid, &pbss_desc->mac_address, MLAN_MAC_ADDR_LENGTH);
    ...
    memcpy(pmadapter, &info->param.bss_info.capability_info, &pbss_desc->cap_info,
    ...
    memset(pmadapter, info->param.bss_info.peer_supp_rates, 0, sizeof(info->param.bss_info.peer_supp_rates));
    memcpy(pmadapter, info->param.bss_info.peer_supp_rates, pbss_desc->supported_rates,
...
static mlan_status wlan_get_info_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    switch (pget_info->sub_command)
    {
        ...
        case MLAN_OID_GET_FW_INFO:
            ...
            memcpy(pmadapter, &pget_info->param.fw_info.mac_addr, pmpriv->curr_addr, MLAN_MAC_ADDR_LENGTH);
...
static mlan_status wlan_bss_ioctl_mac_address(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        ...
        memcpy(pmadapter, &bss->param.mac_addr, pmpriv->curr_addr, MLAN_MAC_ADDR_LENGTH);
        ret = MLAN_STATUS_SUCCESS;
        goto exit;
    }

    memcpy(pmadapter, pmpriv->curr_addr, &bss->param.mac_addr, MLAN_MAC_ADDR_LENGTH);
...
static mlan_status wlan_query_passphrase(mlan_private *priv, pmlan_ioctl_req pioctl_req)
{
    ...
    if (ssid_bssid->ssid.ssid_len == 0)
    {
        ...
        if (i >= 0)
        {
            ...
            memcpy(pmadapter, &sec_pp.ssid, &pbss_desc->ssid, sizeof(mlan_802_11_ssid));
        }
        else
            memcpy(pmadapter, &sec_pp.bssid, &ssid_bssid->bssid, MLAN_MAC_ADDR_LENGTH);
    }
    else
    {
        memcpy(pmadapter, &sec_pp.ssid, &ssid_bssid->ssid, sizeof(mlan_802_11_ssid));
    }
...
static mlan_status wlan_sec_ioctl_set_wpa_key(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (sec->param.encrypt_key.key_len == WPA_AES_KEY_LEN)
    {
        ...
        memcpy(pmpriv->adapter, (t_u8 *)&pmpriv->aes_key, (t_u8 *)&sec->param.encrypt_key, sizeof(pmpriv->aes_key));
#else
        memset(pmpriv->adapter, pmpriv->aes_key.key_param_set.key, 0, sizeof(pmpriv->aes_key.key_param_set.key));
        pmpriv->aes_key.key_param_set.key_len = sec->param.encrypt_key.key_len;
        memcpy(pmpriv->adapter, pmpriv->aes_key.key_param_set.key, sec->param.encrypt_key.key_material,
    ...
    if ((pmpriv->bss_mode == MLAN_BSS_MODE_IBSS) && (sec->param.encrypt_key.key_len == WPA_AES_KEY_LEN))
    {
        if (memcmp(pmadapter, sec->param.encrypt_key.mac_addr, broadcast_mac_addr, MLAN_MAC_ADDR_LENGTH) == 0)
        {
            ...
            memcpy(pmadapter, sec->param.encrypt_key.mac_addr, pmpriv->curr_addr, MLAN_MAC_ADDR_LENGTH);
        }
...
static mlan_status wlan_sec_ioctl_get_key(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
        ...
        (sec->param.encrypt_key.key_len == WPA_AES_KEY_LEN))
    {
        if (pmpriv->adhoc_aes_enabled == MTRUE && (pmpriv->aes_key.key_param_set.key_len == WPA_AES_KEY_LEN))
        {
            ...
            memcpy(pmadapter, sec->param.encrypt_key.key_material, pmpriv->aes_key.key_param_set.key, WPA_AES_KEY_LEN);
            LEAVE();
            return ret;
        }
        else
    ...
    if (sec->param.encrypt_key.key_index == MLAN_KEY_INDEX_DEFAULT)
    {
            ...
            (pmpriv->sec_info.wep_status == Wlan802_11WEPEnabled))
        {
            ...
            memcpy(pmadapter, sec->param.encrypt_key.key_material, pmpriv->wep_key[index].key_material,
                 ...
                 (pmpriv->sec_info.wapi_enabled) || (pmpriv->adhoc_aes_enabled))
        {
            /* Return WPA enabled */
            sec->param.encrypt_key.key_disable = MFALSE;
            memcpy(pmadapter, sec->param.encrypt_key.key_material, pmpriv->aes_key.key_param_set.key,
    ...
    else
    {
        if (sec->param.passphrase.psk_type == MLAN_PSK_QUERY)
        {
            if (sec->param.passphrase.ssid.ssid_len == 0)
            {
                ...
                {
                    pbss_desc = &pmadapter->pscan_table[i];
                    memcpy(pmadapter, &sec->param.passphrase.ssid, &pbss_desc->ssid, sizeof(mlan_802_11_ssid));
...
static int wlan_set_gen_ie_helper(mlan_private *priv, t_u8 *ie_data_ptr, t_u16 ie_len)
{
    ...
    else
    {
                 ...
                 (!memcmp(priv->adapter, pvendor_ie->oui, wps_oui, sizeof(wps_oui))))
        {
            ...
            if (pvendor_ie->len > 4)
            {
                memcpy(priv->adapter, (t_u8 *)&priv->wps.wps_ie, ie_data_ptr, ie_len);
        ...
        else
        {
            ...
            if (ie_len < (sizeof(priv->gen_ie_buf) - priv->gen_ie_buf_len))
            {
                ...
                memcpy(priv->adapter, priv->gen_ie_buf + priv->gen_ie_buf_len, ie_data_ptr, ie_len);
    ...
    switch (misc->param.gen_ie.type)
    {
        case MLAN_IE_TYPE_GEN_IE:
            if (pioctl_req->action == MLAN_ACT_GET)
            {
                ...
                memcpy(pmadapter, misc->param.gen_ie.ie_data, pmpriv->wpa_ie, misc->param.gen_ie.len);
            }
            else
            {
        ...
        case MLAN_IE_TYPE_ARP_FILTER:
            ...
            {
                memcpy(pmadapter, pmadapter->arp_filter, misc->param.gen_ie.ie_data, misc->param.gen_ie.len);
...
mlan_status wlan_ipaddr_auto_arp_resp(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req, IN t_u32 ipv4_addr)
{
    ...
    else
    {
        ...
        memcpy(pmpriv->adapter, &filter[FLTR_BUF_IP_OFFSET], &ipv4_addr, sizeof(ipv4_addr));
...
mlan_status wlan_misc_ioctl_mef_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    switch (mef_cfg->sub_id)
    {
        ...
        case MEF_CFG_HOSTCMD:
            ...
            filter = buf + buf_len;
            memcpy(pmpriv->adapter, filter, mef_cfg->param.cmd_buf.cmd, mef_cfg->param.cmd_buf.len);
...
mlan_status wlan_misc_ioctl_ipaddr_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        memcpy(pmadapter, misc->param.ipaddr_cfg.ip_addr, pmpriv->ip_addr, IPADDR_LEN);
        misc->param.ipaddr_cfg.op_code = pmpriv->op_code;
        goto done;
    }
    /* only one IP is supported in current firmware */
    memcpy(pmadapter, &ipv4_addr, misc->param.ipaddr_cfg.ip_addr[0], sizeof(t_u32));
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        ...
        memcpy(pmadapter, pmpriv->ip_addr, misc->param.ipaddr_cfg.ip_addr, IPADDR_LEN);
    }
...
mlan_status wlan_misc_ioctl_country_code(IN pmlan_adapter pmadapter, IN mlan_ioctl_req *pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        ...
        memcpy(pmadapter, pmadapter->country_code, country_code->country_code, COUNTRY_CODE_LEN);
    }
    else
    {
        /* GET operation */
        memcpy(pmadapter, country_code->country_code, pmadapter->country_code, COUNTRY_CODE_LEN);
    }
...
mlan_status wlan_find_bss(mlan_private *pmpriv, pmlan_ioctl_req pioctl_req)
{
    ...
    if (memcmp(pmadapter, &bss->param.ssid_bssid.bssid, zero_mac, sizeof(zero_mac)))
    {
        ...
        pbss_desc = &pmadapter->pscan_table[i];
        memcpy(pmadapter, &bss->param.ssid_bssid.ssid, &pbss_desc->ssid, sizeof(mlan_802_11_ssid));
    ...
    else if (bss->param.ssid_bssid.ssid.ssid_len)
    {
        ...
        pbss_desc = &pmadapter->pscan_table[i];
        memcpy(pmadapter, (t_u8 *)&bss->param.ssid_bssid.bssid, (t_u8 *)&pbss_desc->mac_address, MLAN_MAC_ADDR_LENGTH);
        /* index in bss list, start from 1 */
        bss->param.ssid_bssid.idx = i + 1;
    }
    else
