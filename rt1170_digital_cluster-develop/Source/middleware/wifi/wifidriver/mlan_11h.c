...
static mlan_status wlan_11h_cmd_tpc_request(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    ...
    memcpy(priv->adapter, &pcmd_ptr->params.tpc_req, pinfo_buf, sizeof(HostCmd_DS_802_11_TPC_ADAPT_REQ));
...
static mlan_status wlan_11h_cmd_chan_sw_ann(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    ...
    memcpy(priv->adapter, &pcmd_ptr->params.chan_sw_ann, pch_sw_ann, sizeof(HostCmd_DS_802_11_CHAN_SW_ANN));
...
static mlan_status wlan_11h_cmd_chan_rpt_req(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    ...
    memcpy(priv->adapter, &pcmd_ptr->params.chan_rpt_req, pchan_rpt_req, sizeof(HostCmd_DS_CHAN_RPT_REQ));
                                    ...
                                    wlan_11h_bss_info_t *p11h_bss_info)
{
    ...
    if (p11h_bss_info)
    {
         ...
         */
        memcpy(adapter, dfs_elem.dfs_owner, p11h_bss_info->ibss_dfs.dfs_owner, sizeof(dfs_elem.dfs_owner));
...
mlan_status wlan_11h_cmdresp_process(mlan_private *priv, const HostCmd_DS_COMMAND *resp)
{
    ...
    switch (resp->command)
    {
        case HostCmd_CMD_802_11_TPC_ADAPT_REQ:
            ...
            memcpy(priv->adapter, priv->adapter->curr_cmd->pdata_buf, &resp->params.tpc_req,
        ...
        case EVENT_RADAR_DETECTED:
            ...
            if (pmpriv)
            {
                ...
                PRINTM(MINFO, "%s: priv - bss_num=%d, bss_type=%d\n", __FUNCTION__, GET_BSS_NUM(pmpriv),
                       pmpriv->bss_type);
                memcpy(pmadapter, pmevbuf->pbuf + pmevbuf->data_offset, &new_event_cause, sizeof(new_event_cause));
                ret = MLAN_STATUS_SUCCESS;
            }
            break;
    }
