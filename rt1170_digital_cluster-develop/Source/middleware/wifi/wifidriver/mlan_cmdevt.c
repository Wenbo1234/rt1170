...
static mlan_status wlan_cmd_host_cmd(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    memcpy(pmpriv->adapter, (void *)cmd, pcmd_ptr->cmd, MIN(MRVDRV_SIZE_OF_CMD_BUFFER, pcmd_ptr->len));
...
mlan_status wlan_process_event(pmlan_adapter pmadapter)
{
    ...
    if ((eventcause & EVENT_ID_MASK) == EVENT_RADAR_DETECTED)
    {
        ...
        {
            memcpy(pmadapter, (t_u8 *)&eventcause, pmbuf->pbuf + pmbuf->data_offset, sizeof(eventcause));
        }
    }
    ...
    if (pmbuf)
    {
        ...
        memcpy(pmadapter, pmbuf->pbuf + pmbuf->data_offset, (t_u8 *)&eventcause, sizeof(eventcause));
    }
...
mlan_status wlan_ret_ver_ext(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *resp, IN mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        memcpy(pmpriv->adapter, info->param.ver_ext.version_str, ver_ext->version_str, sizeof(char) * 128);
    }
    LEAVE();
    return MLAN_STATUS_SUCCESS;
}
                                          ...
                                          IN mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        if (rate->sub_command == MLAN_OID_RATE_CFG)
        {
            ...
            {
                rate->param.rate_cfg.rate = wlan_index_to_data_rate(pmadapter, pmpriv->tx_rate, pmpriv->tx_htinfo);
            }
        }
                                 ...
                                 IN t_void *pdata_buf)
{
    ...
    if (pbitmap_rates != MNULL)
    {
        ...
        for (i = 0; i < sizeof(rate_scope->ht_mcs_rate_bitmap) / sizeof(t_u16); i++)
 ...
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_ret_tx_rate_cfg(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *resp, IN mlan_ioctl_req *pioctl_buf)
{
    mlan_adapter *pmadapter = pmpriv->adapter;
...
#ifndef CONFIG_MLAN_WMSDK
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_TX_RATE_QUERY, HostCmd_ACT_GEN_GET, 0, MNULL, MNULL);
#endif /* CONFIG_MLAN_WMSDK */
    }
...
#ifdef CONFIG_P2P
        memmove(pmadapter, pmpriv->curr_p2p_addr, hw_spec->permanent_addr, MLAN_MAC_ADDR_LENGTH);
