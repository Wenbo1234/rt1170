                                          ...
                                          IN pmlan_ioctl_req pioctl_buf)
{
        ...
        (bss->param.bss_config.filter.mac_count <= MAX_MAC_FILTER_NUM))
    {
        ...
        memcpy(pmpriv->adapter, tlv_mac_filter->mac_address, (t_u8 *)bss->param.bss_config.filter.mac_list,
    ...
    else
    {
             ...
             (bss->param.bss_config.wep_cfg.key0.length == 13) || (bss->param.bss_config.wep_cfg.key0.length == 26)))
        {
            ...
            memcpy(pmpriv->adapter, tlv_wep_key->key, bss->param.bss_config.wep_cfg.key0.key,
             ...
             (bss->param.bss_config.wep_cfg.key1.length == 13) || (bss->param.bss_config.wep_cfg.key1.length == 26)))
        {
            ...
            memcpy(pmpriv->adapter, tlv_wep_key->key, bss->param.bss_config.wep_cfg.key1.key,
             ...
             (bss->param.bss_config.wep_cfg.key2.length == 13) || (bss->param.bss_config.wep_cfg.key2.length == 26)))
        {
            ...
            memcpy(pmpriv->adapter, tlv_wep_key->key, bss->param.bss_config.wep_cfg.key2.key,
             ...
             (bss->param.bss_config.wep_cfg.key3.length == 13) || (bss->param.bss_config.wep_cfg.key3.length == 26)))
        {
            ...
            memcpy(pmpriv->adapter, tlv_wep_key->key, bss->param.bss_config.wep_cfg.key3.key,
    ...
    if (bss->param.bss_config.wmm_para.qos_info == 0x80 || bss->param.bss_config.wmm_para.qos_info == 0x00)
    {
        ...
        memcpy(pmpriv->adapter, tlv_wmm_parameter->wmm_para.ouitype, bss->param.bss_config.wmm_para.ouitype,
                                              ...
                                              IN t_void *pdata_buf)
{
            ...
            switch (*(t_u16 *)pdata_buf)
            {
                ...
                case TLV_TYPE_MGMT_IE:
                    ...
                    if (ie && cust_ie->ie_data_list)
                    {
                        ...
                        memcpy(pmpriv->adapter, ie, cust_ie->ie_data_list, cust_ie->len);
                    }
                    break;
#endif /* CONFIG_MLAN_WMSDK */
                default:
    ...
    if (pioctl_buf->req_id == MLAN_IOCTL_BSS)
    {
        ...
        if (bss->sub_command == MLAN_OID_BSS_MAC_ADDR)
        {
            ...
            mac_tlv->header.len  = wlan_cpu_to_le16(MLAN_MAC_ADDR_LENGTH);
            if (cmd_action == HostCmd_ACT_GEN_SET)
                memcpy(pmpriv->adapter, mac_tlv->mac, &bss->param.mac_addr, MLAN_MAC_ADDR_LENGTH);
#endif /* CONFIG_MLAN_WMSDK */
        }
    ...
    else if (pioctl_buf->req_id == MLAN_IOCTL_MISC_CFG)
    {
        ...
        if ((misc->sub_command == MLAN_OID_MISC_GEN_IE) && (misc->param.gen_ie.type == MLAN_IE_TYPE_GEN_IE))
        {
            ...
            if (cmd_action == HostCmd_ACT_GEN_SET)
                memcpy(pmpriv->adapter, ie, misc->param.gen_ie.ie_data, misc->param.gen_ie.len);
        }
        if ((misc->sub_command == MLAN_OID_MISC_CUSTOM_IE) && (misc->param.cust_ie.type == TLV_TYPE_MGMT_IE))
        {
            ...
            if (ie && misc->param.cust_ie.ie_data_list)
            {
                ...
                if (misc->param.cust_ie.len)
                    memcpy(pmpriv->adapter, ie, misc->param.cust_ie.ie_data_list, misc->param.cust_ie.len);
            }
        }
#endif /* CONFIG_MLAN_WMSDK */
    }
    ...
    while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t))
    {
        ...
        switch (tlv_type)
        {
            case TLV_TYPE_UAP_MAC_ADDRESS:
                tlv_mac = (MrvlIEtypes_MacAddr_t *)tlv;
                memcpy(pmpriv->adapter, &bss->param.bss_config.mac_addr, tlv_mac->mac, MLAN_MAC_ADDR_LENGTH);
                break;
            case TLV_TYPE_SSID:
                tlv_ssid                            = (MrvlIEtypes_SsIdParamSet_t *)tlv;
                bss->param.bss_config.ssid.ssid_len = MIN(MLAN_MAX_SSID_LENGTH, tlv_len);
                memcpy(pmpriv->adapter, bss->param.bss_config.ssid.ssid, tlv_ssid->ssid,
            ...
            case TLV_TYPE_RATES:
                tlv_rates = (MrvlIEtypes_RatesParamSet_t *)tlv;
                memcpy(pmpriv->adapter, bss->param.bss_config.rates, tlv_rates->rates, MIN(MAX_DATA_RATES, tlv_len));
                break;
            ...
            case TLV_TYPE_UAP_STA_MAC_ADDR_FILTER:
                ...
                memcpy(pmpriv->adapter, (t_u8 *)bss->param.bss_config.filter.mac_list, tlv_mac_filter->mac_address,
                       MLAN_MAC_ADDR_LENGTH * bss->param.bss_config.filter.mac_count);
                break;
            case TLV_TYPE_UAP_CHAN_BAND_CONFIG:
                tlv_chan_band                         = (MrvlIEtypes_channel_band_t *)tlv;
            ...
            case TLV_TYPE_UAP_WPA_PASSPHRASE:
                ...
                memcpy(pmpriv->adapter, bss->param.bss_config.wpa_cfg.passphrase, tlv_passphrase->passphrase,
                       bss->param.bss_config.wpa_cfg.length);
                break;
#ifdef WIFI_DIRECT_SUPPORT
            case TLV_TYPE_UAP_PSK:
                tlv_psk = (MrvlIEtypes_psk_t *)tlv;
                memcpy(pmpriv->adapter, bss->param.bss_config.psk, tlv_psk->psk, MIN(MLAN_MAX_KEY_LENGTH, tlv_len));
                break;
#endif /* WIFI_DIRECT_SUPPORT */
            ...
            case TLV_TYPE_UAP_WEP_KEY:
                ...
                if (pkey)
                {
                    ...
                    memcpy(pmpriv->adapter, pkey->key, tlv_wep_key->key, pkey->length);
                }
                break;
            ...
            case HT_CAPABILITY:
                ...
                memcpy(pmpriv->adapter, bss->param.bss_config.supported_mcs_set, tlv_htcap->ht_cap.supported_mcs_set,
                       16);
                                           ...
                                           IN mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        if (pioctl_buf->req_id == MLAN_IOCTL_BSS)
        {
            ...
            if (bss->sub_command == MLAN_OID_BSS_MAC_ADDR)
            {
                ...
                {
                    memcpy(pmpriv->adapter, &bss->param.mac_addr, tlv->mac, MLAN_MAC_ADDR_LENGTH);
                }
            }
    ...
    { /* no ioctl: driver generated get/set */
        ...
        {
            case TLV_TYPE_UAP_MAC_ADDRESS:
                memcpy(pmpriv->adapter, pmpriv->curr_addr, tlv->mac, MLAN_MAC_ADDR_LENGTH);
                break;
            case TLV_TYPE_UAP_CHAN_BAND_CONFIG:
...
static mlan_status wlan_uap_cmd_report_mic(pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    memcpy(pmpriv->adapter, pcmd_report_mic->mac, pdata_buf, MLAN_MAC_ADDR_LENGTH);
    LEAVE();
    return MLAN_STATUS_SUCCESS;
}
    ...
    IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_u16 cmd_action, IN t_u16 cmd_oid, IN t_void *pdata_buf)
{
    ...
    if (pkey->is_wapi_key)
    {
        ...
        memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.wapi.pn, pkey->pn, PN_SIZE);
        pkey_material->key_param_set.key_params.wapi.key_len = MIN(WAPI_KEY_SIZE, wlan_cpu_to_le16(pkey->key_len));
        memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.wapi.key, pkey->key_material,
    ...
    IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_u16 cmd_action, IN t_u16 cmd_oid, IN t_void *pdata_buf)
{
    ...
    memset(pmpriv->adapter, &pkey_material->key_param_set, 0, sizeof(MrvlIEtype_KeyParamSet_t));
#ifndef CONFIG_MLAN_WMSDK
#ifdef WAPI
    if (pkey->is_wapi_key)
    {
        ...
        memcpy(pmpriv->adapter, &pkey_material->key_param_set.key[2], pkey->key_material, pkey->key_len);
    ...
    if (pkey_material->key_param_set.key_type_id)
    {
        ...
        memcpy(pmpriv->adapter, pkey_material->key_param_set.key, pkey->key_material, pkey->key_len);
        ...
        if (pkey_material->key_param_set.key_type_id == wlan_cpu_to_le16(KEY_TYPE_ID_AES_CMAC))
        {
            ...
            memcpy(pmpriv->adapter, param->key, pkey->key_material, pkey->key_len);
                                         ...
                                         IN mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        {
            memcpy(pmpriv->adapter, info->param.sta_list.info[i].mac_address, tlv->mac_address, MLAN_MAC_ADDR_LENGTH);
...
static void wlan_check_sta_capability(pmlan_private priv, pmlan_buffer pevent, sta_node *sta_ptr)
{
    ...
    while (tlv_buf_left >= (int)sizeof(MrvlIEtypesHeader_t))
    {
        ...
        if (tlv_type == TLV_TYPE_UAP_MGMT_FRAME)
        {
            ...
            memcpy(priv->adapter, &frame_control, (t_u8 *)&(mgmt_tlv->frame_control), sizeof(frame_control));
...
static t_u32 wlan_update_wapi_info_tlv(pmlan_private priv, pmlan_buffer pevent)
{
    ...
    while (tlv_buf_left >= (int)sizeof(MrvlIEtypesHeader_t))
    {
        ...
        if (tlv_type == TLV_TYPE_AP_WAPI_INFO)
        {
            ...
            memcpy(priv->adapter, (t_u8 *)tx_pn, wapi_tlv->multicast_PN, PN_SIZE);
            for (i = 0; i < 4; i++)
                tx_pn[i] = mlan_ntohl(tx_pn[i]);
            memcpy(priv->adapter, wapi_tlv->multicast_PN, (t_u8 *)tx_pn, PN_SIZE);
...
static t_u32 wlan_process_sta_assoc_event(pmlan_private priv, mlan_event *pevent, pmlan_buffer pmbuf)
{
    ...
    memcpy(priv->adapter, pevent->event_buf, pmbuf->pbuf + pmbuf->data_offset + 6, pevent->event_len);
    while (tlv_buf_left >= (int)sizeof(MrvlIEtypesHeader_t))
    {
        tlv_type = wlan_le16_to_cpu(tlv->type);
        tlv_len  = wlan_le16_to_cpu(tlv->len);
        ...
        if (tlv_type == TLV_TYPE_UAP_MGMT_FRAME)
        {
                ...
                ((frame_sub_type == SUBTYPE_ASSOC_REQUEST) || (frame_sub_type == SUBTYPE_REASSOC_REQUEST)))
            {
                ...
                ie_len       = tlv_len - sizeof(IEEEtypes_FrameCtl_t) - assoc_ie_len;
                assoc_req_ie = (t_u8 *)tlv + sizeof(MrvlIETypes_MgmtFrameSet_t) + assoc_ie_len;
                memcpy(priv->adapter, pevent->event_buf + pevent->event_len, assoc_req_ie, ie_len);
...
mlan_status wlan_ops_uap_process_event(IN t_void *priv)
{
    ...
    switch (eventcause)
    {
        case EVENT_MICRO_AP_BSS_START:
            ...
            memcpy(pmadapter, pmpriv->curr_addr, pmadapter->event_body + 2, MLAN_MAC_ADDR_LENGTH);
        ...
        case EVENT_MICRO_AP_STA_ASSOC:
            ...
            memcpy(pmadapter, sta_addr, pmadapter->event_body + 2, MLAN_MAC_ADDR_LENGTH);
        ...
        case EVENT_MICRO_AP_STA_DEAUTH:
            ...
            memcpy(pmadapter, (t_u8 *)pevent->event_buf, pmbuf->pbuf + pmbuf->data_offset + 4, pevent->event_len);
        ...
        case EVENT_RADAR_DETECTED:
            ...
            memcpy(pmadapter, (t_u8 *)pevent->event_buf, pmbuf->pbuf + pmbuf->data_offset, pevent->event_len);
            wlan_recv_event(pmpriv, pevent->event_id, pevent);
        ...
        case EVENT_CHANNEL_REPORT_RDY:
            ...
            /* Copy event data */
            memcpy(pmadapter, (t_u8 *)pevent->event_buf, pmbuf->pbuf + pmbuf->data_offset + sizeof(eventcause),
                   pevent->event_len);
