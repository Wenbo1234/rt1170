...
static void wlan_add_wps_probe_request_ie(IN mlan_private *pmpriv, OUT t_u8 **pptlv_out)
{
    ...
    if (pmpriv->wps.wps_ie.vend_hdr.len)
    {
        ...
        *pptlv_out += sizeof(MrvlIEtypesHeader_t);
        memcpy(pmpriv->adapter, *pptlv_out, pmpriv->wps.wps_ie.vend_hdr.oui, pmpriv->wps.wps_ie.vend_hdr.len);
                                                ...
                                                IN BSSDescriptor_t *pnew_beacon)
{
    ...
    if (pmadapter->pscan_table[beacon_idx].pbeacon_buf)
    {
        ...
        if (new_bcn_size == old_bcn_size)
        {
             ...
             */
            memcpy(pmadapter, pbcn_store, pnew_beacon->pbeacon_buf, pnew_beacon->beacon_buf_size);
        }
        else if (new_bcn_size <= bcn_space)
        {
            ...
            memcpy(pmadapter, pbcn_store, pnew_beacon->pbeacon_buf, new_bcn_size);
    ...
    else
    {
            ...
            (pmadapter->bcn_buf + pmadapter->bcn_buf_size))
        {
             ...
             */
            memcpy(pmadapter, pmadapter->pbcn_buf_end, pnew_beacon->pbeacon_buf, pnew_beacon->beacon_buf_size);
...
static t_void wlan_restore_curr_bcn(IN mlan_private *pmpriv)
{
        ...
        ((pmadapter->pbcn_buf_end + pmpriv->curr_bcn_size) < (pmadapter->bcn_buf + pmadapter->bcn_buf_size)))
    {
        ...
        memcpy(pmadapter, pmadapter->pbcn_buf_end, pmpriv->pcurr_bcn_buf, pmpriv->curr_bcn_size);
...
/* static */ t_void wlan_scan_process_results(IN mlan_private *pmpriv)
{
    ...
    if (pmpriv->media_connected == MTRUE)
    {
        ...
        if (j >= 0)
        {
            ...
            memcpy(pmadapter, &pmpriv->curr_bss_params.bss_descriptor, &pmadapter->pscan_table[j],
...
static t_void wlan_scan_delete_table_entry(IN mlan_private *pmpriv, IN t_s32 table_idx)
{
    ...
    for (del_idx = table_idx; (del_idx + 1) < pmadapter->num_in_scan_table; del_idx++)
    {
        ...
        memcpy(pmadapter, pmadapter->pscan_table + del_idx, pmadapter->pscan_table + del_idx + 1,
    ...
    memset(pmadapter, pmadapter->pscan_table + pmadapter->num_in_scan_table - 1, 0x00, sizeof(BSSDescriptor_t));
...
mlan_status wlan_cmd_bgscan_config(IN mlan_private *pmpriv, IN HostCmd_DS_COMMAND *pcmd, IN t_void *pdata_buf)
{
         ...
         ssid_idx++)
    {
        ...
        memcpy(pmadapter, pwildcard_ssid_tlv->ssid, bg_scan_in->ssid_list[ssid_idx].ssid,
               MIN(MLAN_MAX_SSID_LENGTH, ssid_len));
...
mlan_status wlan_find_best_network(IN mlan_private *pmpriv, OUT mlan_ssid_bssid *preq_ssid_bssid)
{
    ...
    if (i >= 0)
    {
        ...
        memcpy(pmadapter, &preq_ssid_bssid->ssid, &preq_bss->ssid, sizeof(mlan_802_11_ssid));
        memcpy(pmadapter, (t_u8 *)&preq_ssid_bssid->bssid, (t_u8 *)&preq_bss->mac_address, MLAN_MAC_ADDR_LENGTH);
...
mlan_status wlan_scan_specific_ssid(IN mlan_private *pmpriv, IN t_void *pioctl_buf, IN mlan_802_11_ssid *preq_ssid)
{
    ...
    memcpy(pmpriv->adapter, pscan_cfg->ssid_list[0].ssid, preq_ssid->ssid, preq_ssid->ssid_len);
...
t_void wlan_save_curr_bcn(IN mlan_private *pmpriv)
{
        ...
        (memcmp(pmpriv->adapter, pmpriv->pcurr_bcn_buf, pcurr_bss->pbeacon_buf, pcurr_bss->beacon_buf_size)))
    {
        ...
        if (pmpriv->curr_bcn_size)
        {
            ...
            {
                memcpy(pmpriv->adapter, pmpriv->pcurr_bcn_buf, pcurr_bss->pbeacon_buf, pcurr_bss->beacon_buf_size);
