...
static mlan_status wlan_11n_ioctl_addba_reject(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        ...
        memcpy(pmadapter, cfg->param.addba_reject, pmpriv->addba_reject, MAX_NUM_TID);
    }
    else
    {
...
int wlan_cmd_append_11n_tlv(IN mlan_private *pmpriv, IN BSSDescriptor_t *pbss_desc, OUT t_u8 **ppbuffer)
{
    ...
    if ((pbss_desc->bss_band & (BAND_B | BAND_G)) && ISSUPP_CHANWIDTH40(usr_dot_11n_dev_cap) &&
...
int wlan_get_rxreorder_tbl(mlan_private *priv, rx_reorder_tbl *buf)
{
    ...
    while (rxReorderTblPtr != (RxReorderTbl *)&priv->rx_reorder_tbl_ptr)
    {
        ...
        memcpy(priv->adapter, ptbl->ta, rxReorderTblPtr->ta, MLAN_MAC_ADDR_LENGTH);
