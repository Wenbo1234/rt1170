...
static mlan_status wrapper_moal_recv_packet(IN t_void *pmoal_handle, IN pmlan_buffer pmbuf)
{
    ...
    w_pkt_d("[push] %p + %d:  DELV: %p L: %d", pmbuf->pbuf, pmbuf->data_offset, p, pmbuf->data_len);
    deliver_packet_above(p, prx_pd->bss_type);
...
int wrapper_wlan_cmd_11d_enable(HostCmd_DS_COMMAND *cmd)
{
    ...
    cmd->seq_num = 0x0;
    cmd->result  = 0x0;
    return wlan_ops_sta_prepare_cmd((mlan_private *)mlan_adap->priv[0], HostCmd_CMD_802_11_SNMP_MIB,
                                    HostCmd_ACT_GEN_SET, Dot11D_i, NULL, &enable_11d, cmd);
}
...
static void compute_data_rate(const mlan_private *pmpriv,
                              const HostCmd_TX_RATE_QUERY *datarate,
                              wifi_data_rate_t *data_rate)
{
    if (datarate->ht_info & MBIT(0))
    {
        /* HT rates detected */
        data_rate->tx_data_rate = datarate->tx_rate + MLAN_RATE_INDEX_MCS0;
        if (datarate->ht_info & MBIT(1))
            data_rate->tx_ht_bw = MLAN_HT_BW40;
        else
            data_rate->tx_ht_bw = MLAN_HT_BW20;
        if (datarate->ht_info & MBIT(2))
            data_rate->tx_ht_gi = MLAN_HT_SGI;
        else
            data_rate->tx_ht_gi = MLAN_HT_LGI;
    }
    else
    {
        /*
        ...
        data_rate->tx_data_rate =
            (datarate->tx_rate > MLAN_RATE_INDEX_OFDM0) ? datarate->tx_rate - 1 : datarate->tx_rate;
    }
    if (pmpriv->rxpd_htinfo & MBIT(0))
    {
        data_rate->rx_data_rate = pmpriv->rxpd_rate + MLAN_RATE_INDEX_MCS0;
        if (pmpriv->rxpd_htinfo & MBIT(1))
            data_rate->rx_ht_bw = MLAN_HT_BW40;
        else
            data_rate->rx_ht_bw = MLAN_HT_BW20;
        if (pmpriv->rxpd_htinfo & MBIT(2))
            data_rate->rx_ht_gi = MLAN_HT_SGI;
        else
            data_rate->rx_ht_gi = MLAN_HT_LGI;
    }
    else
    {
        /*
...
static void load_bss_list(const HostCmd_DS_STA_LIST *sta_list)
{
    ...
    *(sta_list_t **)wm_wifi.cmd_resp_priv = NULL;
    ...
    sta_list_t *sl = os_mem_alloc(sizeof(sta_list_t) + c * sizeof(sta_info_t));

    if (!sl)
        return;

    sl->count       = c;
    sta_info_t *sta = (sta_info_t *)(((t_u8 *)&sl->count) + sizeof(int));
...
#if 0
    HostCmd_DS_802_11_RF_ANTENNA *antcfg = (HostCmd_DS_802_11_RF_ANTENNA *)&resp->params;
...
int wifi_process_cmd_response(HostCmd_DS_COMMAND *resp)
{
    ...
    switch (command)
    {
        ...
        case HostCmd_CMD_802_11_TX_RATE_QUERY:
        {
            ...
            if (wm_wifi.cmd_resp_priv)
            {
                wifi_data_rate_t *data_rate = (wifi_data_rate_t *)wm_wifi.cmd_resp_priv;
                compute_data_rate(pmpriv, &resp->params.tx_rate, data_rate);
            }
            wm_wifi.cmd_resp_status = WM_SUCCESS;
        }
        break;
        ...
        case HostCmd_CMD_CHANNEL_TRPC_CONFIG:
        {
            ...
            if (resp->result == HostCmd_RESULT_OK)
            {
                if (chan_trpc_cfg->action == HostCmd_ACT_GEN_GET)
                {
                    ...
                    {
                        int i;
                        chan_trpc_t *trpc_config = (chan_trpc_t *)wm_wifi.cmd_resp_priv;

                        trpc_config->num_chans = 14;
                        for (i = 0; i < 14; i++)
                        {
                            trpc_config->chan_trpc_config[i].num_mod_grps = 7;
                            memcpy(&trpc_config->chan_trpc_config[i].chan_desc,
                                   ((uint8_t *)&chan_trpc_cfg->ChanTrpcG.ChanTrpc[i].chanDesc + (i * 6)),
                                   sizeof(MrvlChannelDesc_t));
                            memcpy(trpc_config->chan_trpc_config[i].chan_trpc_entry,
                                   ((uint8_t *)chan_trpc_cfg->ChanTrpcG.ChanTrpc[i].chanTrpcEntry + (i * 6)),
                                   7 * sizeof(MrvlChanTrpcEntry_t));
                        }
                    }
                }
                wm_wifi.cmd_resp_status = WM_SUCCESS;
            ...
            if (resp->result == HostCmd_RESULT_OK)
            {
                if (bridge_mode->action == HostCmd_ACT_GEN_GET)
                {
                    if (wm_wifi.cmd_resp_priv)
                    {
                        ...
                        memset(bridgecfg, 0x00, sizeof(wifi_bridge_cfg_t));
                        ...
                        memcpy(bridgecfg->ex_ap_ssid, ssid->ssid, bridgecfg->ex_ap_ssid_len);
                        ...
                        memcpy(bridgecfg->ex_ap_pass, pass->passphrase, bridgecfg->ex_ap_pass_len);
                        ...
                        memcpy(bridgecfg->bridge_ssid, ssid->ssid, bridgecfg->bridge_ssid_len);

                        tlv = (t_u8 *)ssid;

                        pass = (MrvlIEtypes_Passphrase_t *)(tlv + sizeof(MrvlIEtypesHeader_t) + ssid->header.len);
                        bridgecfg->bridge_pass_len = wlan_cpu_to_le16(pass->header.len);
                        memcpy(bridgecfg->bridge_pass, pass->passphrase, bridgecfg->bridge_pass_len);
                        ...
                        {
                            memcpy(&bridgecfg->autolink, ((char *)autolink) + sizeof(MrvlIEtypesHeader_t),
                                   autolink->header.len);
                        }
                    }
                }
...
int wifi_handle_fw_event(struct bus_message *msg)
{
    ...
    switch (evt->event_id)
    {
        ...
        case EVENT_CHANNEL_SWITCH_ANN:
            wifi_event_completion(WIFI_EVENT_CHAN_SWITCH, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
#endif
...
void wifi_get_firmware_ver_ext_from_cmdresp(const HostCmd_DS_COMMAND *resp, uint8_t *fw_ver_ext)
{
    ...
    {
        memcpy(fw_ver_ext, &resp->params.verext.version_str, MLAN_MAX_VER_STR_LEN);
    }
...
void wifi_prepare_txpwrlimit_data_cmd(HostCmd_DS_COMMAND *cmd, int seq_number)
{
    int i, j;

    cmd->command = HostCmd_CMD_CHANNEL_TRPC_CONFIG;
    cmd->size    = sizeof(HostCmd_DS_CHAN_TRPC_CONFIG) + S_DS_GEN -
                ((14 - txpwrlimit_no_of_channels) * sizeof(MrvlIEtypes_ChanTrpcCfg_t));
    cmd->seq_num                     = seq_number;
    cmd->result                      = 0;
    cmd->params.chan_trpc_cfg.action = HostCmd_ACT_GEN_SET;
    for (i = 0; i < txpwrlimit_no_of_channels; i++)
    {
        cmd->params.chan_trpc_cfg.ChanTrpcG.ChanTrpc[i].header.type = TLV_TYPE_CHANNEL_TRPC_CONFIG;
        cmd->params.chan_trpc_cfg.ChanTrpcG.ChanTrpc[i].header.len =
            sizeof(MrvlChannelDesc_t) + (MOD_GROUPS * sizeof(MrvlChanTrpcEntry_t));
        cmd->params.chan_trpc_cfg.ChanTrpcG.ChanTrpc[i].chanDesc.startFreq = 2407;
        cmd->params.chan_trpc_cfg.ChanTrpcG.ChanTrpc[i].chanDesc.chanWidth = 20;
        cmd->params.chan_trpc_cfg.ChanTrpcG.ChanTrpc[i].chanDesc.chanNum =
            *((uint8_t *)txpwrlimit_data + i * (MOD_GROUPS + 1) + 0);
        for (j = 0; j < MOD_GROUPS; j++)
        {
            cmd->params.chan_trpc_cfg.ChanTrpcG.ChanTrpc[i].chanTrpcEntry[j].modGroup = j;
            cmd->params.chan_trpc_cfg.ChanTrpcG.ChanTrpc[i].chanTrpcEntry[j].txPower =
                *((uint8_t *)txpwrlimit_data + i * (MOD_GROUPS + 1) + j + 1);
        }
    }
}
...
void wifi_set_txpwrlimit_data(const uint8_t country, const uint8_t channels, const uint8_t **data)
{
