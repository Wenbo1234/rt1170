...
SDK_ALIGN(uint8_t outbuf[SDK_SIZEALIGN(DATA_BUFFER_SIZE, SDMMC_DATA_BUFFER_ALIGN_CACHE)],
          MAX(SDMMC_DATA_BUFFER_ALIGN_CACHE, SDMMCHOST_DMA_BUFFER_ADDR_ALIGN));
/*! @brief Data read from the card */
#ifdef CONFIG_SDIO_MULTI_PORT_RX_AGGR
SDK_ALIGN(uint8_t inbuf[SDK_SIZEALIGN(32 * DATA_BUFFER_SIZE, SDMMC_DATA_BUFFER_ALIGN_CACHE)],
          MAX(SDMMC_DATA_BUFFER_ALIGN_CACHE, SDMMCHOST_DMA_BUFFER_ADDR_ALIGN));
#else
SDK_ALIGN(uint8_t inbuf[SDK_SIZEALIGN(2 * DATA_BUFFER_SIZE, SDMMC_DATA_BUFFER_ALIGN_CACHE)],
          MAX(SDMMC_DATA_BUFFER_ALIGN_CACHE, SDMMCHOST_DMA_BUFFER_ADDR_ALIGN));
...
#if defined(CONFIG_XZ_DECOMPRESSION)
mlan_status wlan_download_decomp_fw(
    mdev_t *sdio_drv, enum wlan_fw_storage_type st, mdev_t *fl_dev, t_u8 *wlanfw, t_u32 firmwarelen, t_u32 ioport)
{
    t_u32 tx_blocks = 0, txlen = 0, buflen = 0;
    t_u16 len    = 0;
    t_u32 offset = 0;
...
#endif
                if (st == WLAN_FW_IN_RAM)
                    memcpy(sbuf, wlanfw + offset, readlen);
                offset += readlen;
                firmwarelen -= readlen;
            }
                                    ...
                                    const size_t size)
{
    ...
    if (verify_xz_header(buffer) == WM_SUCCESS)
    {
        wifi_io_d(
            ...
            " len: %d",
            firmwarelen);
        ret = wlan_download_decomp_fw(sdio_drv, st, fl_dev, wlanfw, firmwarelen, mlan_adap->ioport);
    }
    else
#endif /* CONFIG_XZ_DECOMPRESSION */
    {
...
static t_u8 *wlan_read_rcv_packet(t_u32 port, t_u32 rxlen, t_u32 rx_blocks, t_u32 *type)
{
    t_u32 blksize = MLAN_SDIO_BLOCK_SIZE;
    uint32_t resp;

    /* addr = 0 fn = 1 */
    sdio_drv_read(NULL, port, 1, rx_blocks, blksize, inbuf, &resp);
...
static int _wlan_set_txpwrlimit_data()
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_txpwrlimit_data_cmd(&sdiopkt->hostcmd, wlan_get_next_seq_num());
...
static int wlan_set_ed_mac_mode()
{
    ...
    wifi_prepare_ed_mac_mode_cmd(&sdiopkt->hostcmd, wlan_get_next_seq_num());
...
mlan_status wlan_send_gen_sdio_cmd(t_u8 *buf, t_u32 buflen)
{
    ...
    sdio_drv_write(NULL, mlan_adap->ioport, 1, 1, buflen, (t_u8 *)outbuf, &resp);
...
int wlan_send_sdio_cmd(t_u8 *buf, t_u32 buflen)
{
    ...
    sdio_drv_write(NULL, mlan_adap->ioport, 1, 1, buflen, (t_u8 *)outbuf, &resp);
#elif defined(SD8977) || defined(SD8978) || defined(SD8987) || defined(SD8997) || defined(SD9097) || defined(SD9098)
    sdio_drv_write(NULL, mlan_adap->ioport | CMD_PORT_SLCT, 1, 1, buflen, (t_u8 *)outbuf, &resp);
#endif
...
mlan_status wlan_get_rd_port(mlan_adapter *pmadapter, t_u32 *pport, t_u32 *rxlen, t_u32 *rxblocks)
{
#if defined(SD8801)
...
static mlan_status _handle_sdio_packet_read(mlan_adapter *pmadapter, t_u8 **packet, t_u32 *datalen, t_u32 *pkt_type)
{
    ...
    *packet = wlan_read_rcv_packet(port, rx_len, rx_blocks, pkt_type);
    ...
    mlan_adapter *pmadapter, t_u8 **packet, t_u16 *datalen, t_u32 *pkt_type, t_u32 rx_len, t_u32 rx_blocks)
{
    ...
    *packet = wlan_read_rcv_packet(mlan_adap->ioport | CMD_PORT_SLCT, rx_len, rx_blocks, pkt_type);
